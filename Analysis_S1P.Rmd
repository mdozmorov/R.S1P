...
egds.limma <- function(eset, condition, subcondition) {
  mod <- model.matrix(formula(paste("~", condition, "+", 0)), data=pData(eset)[pData(eset)[, "mappings"] == subcondition, ]) # The pData should exist
  colnames(mod) <- group.names
  fit <- lmFit(new("ExpressionSet", exprs=exprs(eset)[, pData(eset)[, "mappings"] == subcondition], featureData=AnnotatedDataFrame(fData(eset))), mod)
