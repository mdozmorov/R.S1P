...
egds <- function(GDS, selected.genes, selected.subset=NA) {
  ...
  # If subset is either "human.tumor" or "PDCCE.tumor", then slice the expression set
  if (!is.na(selected.subset) ) {
    eset <- new("ExpressionSet", exprs=exprs(eset)[, pData(eset)$tissue == selected.subset], phenoData=AnnotatedDataFrame(pData(eset)[pData(eset)$tissue == selected.subset, ]), featureData=AnnotatedDataFrame(fData(eset)))
  }  
...
egds.limma <- function(eset, condition, subcondition) {
  selected.pData <- pData(eset)[pData(eset)[, "mappings"] %in% subcondition, ]
  selected.pData[, "mappings"] <- factor(selected.pData[, "mappings"])
  mod <- model.matrix(formula(paste("~", condition, "+", 0)), data=selected.pData) # The pData should exist
  colnames(mod) <- group.names
  fit <- lmFit(new("ExpressionSet", exprs=exprs(eset)[, pData(eset)[, "mappings"] %in% subcondition], featureData=AnnotatedDataFrame(fData(eset))), mod)
...
# Select subset of data, can be "human tumor" or "PDCCE tumor"
selected.subset <- "human tumor"
# Get data from GEO
eset <- egds(GDS, selected.genes, selected.subset)
```
...
condition <- "mappings" # Manually specify which one to use, must specify 2 groups
table(pData(eset[[1]])[, "mappings"]) # How many samples we have in each group
subcondition <- c("Colon", "Liver")
# Check how many samples are in each condition, given the subcondition
table(pData(eset[[1]])[pData(eset[[1]])[, "mappings"] %in% subcondition, condition])
pData(eset[[1]])[, condition] <- make.names(pData(eset[[1]])[, condition]) # Make names R friendly
group.names <- make.names(names(table(pData(eset[[1]])[pData(eset[[1]])[, "mappings"] %in% subcondition, condition]))) # Keep names of the group
```
...
sapply(rownames(res.selected[order(res.selected$Gene.symbol), ]), function(x) {
  boxplot(exprs(eset[[1]])[x, pData(eset[[1]])$mappings == group.names[1]],
          exprs(eset[[1]])[x, pData(eset[[1]])$mappings == group.names[2]],
...
write.table(res, "results/degs_human_Colon-Liver.txt", sep="\t", row.names=F)
```
